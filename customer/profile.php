...
/* ---------- Formatting Helpers (duplicate of edit_profile) ---------- */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_for_verification'])) {
    // Re-fetch minimal fields needed for checks
    $checkStmt = $conn->prepare(
        "SELECT profile_status, id_front_image, id_back_image, license_front_image, license_back_image
    ...
    if (!$row) {
        ...
        // Only allow if currently unsubmitted or rejected
        if (!in_array($currentStatus, ['unsubmitted','rejected'], true)) {
            $action_err = "Profile cannot be submitted in its current state.";
        } else {
            // Basic completeness check (customize to your needs)
            $missing = [];
            if (empty($row['id_front_image']))      $missing[] = "ID Front Image";
            if (empty($row['id_back_image']))       $missing[] = "ID Back Image";
            if (empty($row['license_front_image'])) $missing[] = "License Front Image";
            if (empty($row['license_back_image']))  $missing[] = "License Back Image";
            if ($missing) {
    ...
    <div class="card">
        ...
        <div class="status-row">
            ...
            <?php if ($statusUpdated): ?>
                <span style="font-size:.68em;color:#6a7489;">
                    Updated: <?= htmlspecialchars($statusUpdated) ?>
                </span>
            <?php endif; ?>
        </div>
